...
class Pooler(MegatronModule):
    ...
    def forward(self, hidden_states, sequence_index=0):
        if torch.distirbuted.get_rank() == 0:
            print(f"mlp")
        # hidden_states: [s, b, h]
...
class Embedding(MegatronModule):
    ...
    def __init__(self,
        ...
        args = get_args()
        self.layer_profile = args.layer_profile
        self.nsys_profile = args.nsys_profile
        self.layer_profile_save_dir = args.layer_profile_save_dir
        self.layer_profile_save = args.layer_profile_save
    ...
    def forward(self, input_ids, position_ids, tokentype_ids=None):
        ...
        #     embedding_layer_time = s.elapsed_time(e)
        #     print(f"[emb] embedding_layer_time: {embedding_layer_time}")
...
class EmbeddingPipe(Embedding):
    ...
    def forward(self, inputs, **kwargs):
        if self.nsys_profile:
            with nvtx.annotate(f"rank {torch.distributed.get_rank()}", color="green"):
                torch.cuda.synchronize()
                ...
                if self.layer_profile:
                    ...
                    print(f"rank{torch.distributed.get_rank()} [embedding] embedding_layer_time: {embedding_layer_time} \n")
            ...
            if self.layer_profile:
                ...
                torch.cuda.synchronize()
                embedding_layer_time = s.elapsed_time(e)
                if self.layer_profile_save:
                    import os
                    import csv
                    csv_file = f"{self.layer_profile_save_dir}/layer_times_rank_{torch.distributed.get_rank()}.csv"
                    file_exists = os.path.isfile(csv_file)
                    with open(csv_file, mode='a', newline='') as file:
                        writer=csv.writer(file)
                        if not file_exists:
                            writer.writerow(['idx','layer_times'])                        
                        writer.writerow(["emb", embedding_layer_time])
                else:
                    print(f"rank{torch.distributed.get_rank()} [embedding] embedding_layer_time: {embedding_layer_time} ")
...
class TransformerLanguageModel(MegatronModule):
    ...
    def __init__(self,
        ...
        if self.layer_profile:
            if torch.distributed.get_rank() == 0:
                os.makedirs(self.layer_profile_save_dir, exist_ok=True)
                print(f"make directory to save layer profile log: {self.layer_profile_save_dir}")
    ...
    def forward(self, enc_input_ids, enc_position_ids, enc_attn_mask,
        ...
        encoder_layer_times = []
        decoder_layer_times = []
        embedding_layer_times = {"encoder_embedding": [], "rotary_embedding": [], "encoder_post_process": [], "decoder_embedding": []}     
        if self.pre_process:
            if self.layer_profile:
                s_emb,e_emb = torch.cuda.Event(enable_timing=True), torch.cuda.Event(enable_timing=True)
                s_emb.record()
            ...
            if self.layer_profile:
                e_emb.record()
                torch.cuda.synchronize()
                embedding_layer_time = s_emb.elapsed_time(e_emb)
                embedding_layer_times["encoder_embedding"].append(embedding_layer_time)
                if torch.distributed.get_rank() == 0:
                    print(f"Transformer Language Model embedding_layer_time: {embedding_layer_time}")
        ...
        # Rotary positional embeddings if llama, it is true
        rotary_pos_emb = None
        if self.use_rotary_position_embeddings:
            if self.layer_profile:
                s_remb, e_remb = torch.cuda.Event(enable_timing=True), torch.cuda.Event(enable_timing=True)
                s_remb.record()
            ...
            if self.layer_profile:
                e_remb.record()
                torch.cuda.synchronize()
                rotary_embedding_layer_time = s_remb.elapsed_time(e_remb)
                embedding_layer_times["rotary_embedding"].append(rotary_embedding_layer_time)
                if torch.distributed.get_rank() == 0:
                    print(f"Transformer Language Model rotary_embedding_layer_time: {rotary_embedding_layer_time}")
        else:
            embedding_layer_times["rotary_embedding"].append(0)
                
        # Run encoder.
        if enc_hidden_states is None:
            if self.encoder is not None:
                encoder_output, *encoder_moe_losses, encoder_layer_times = self.encoder(
                    encoder_input,
                    enc_attn_mask,
            ...
            if self.layer_profile:
                print(f"Transformer Language Model encoder_layer_times: {encoder_layer_times}")    
            else:
        ...
        if self.post_process:
            if self.add_pooler:
                ...
                if self.layer_profile:
                    s_pemb, e_pemb = torch.cuda.Event(enable_timing=True), torch.cuda.Event(enable_timing=True)
                    s_pemb.record()
                ...
                if self.layer_profile:
                    e_pemb.record()
                    torch.cuda.synchronize()
                    post_process_layer_time = s_pemb.elapsed_time(e_pemb)
                    embedding_layer_times["encoder_post_process"].append(post_process_layer_time)
                    if torch.distributed.get_rank() == 0:
                        print(f"Transformer Language Model post_process_layer_time: {post_process_layer_time}")                    
        ...
        if not self.add_decoder or output_enc_hidden:
            if torch.distributed.get_rank() == 0:
                print(f"Transformer Language Model save layer profile log")
            
            if self.layer_profile:
                layer_times = {"encoder_layer_times": encoder_layer_times}
                
                import csv
                layer_header_written=False
                emblayer_header_written=False
                
                layer_times_path=f"{self.layer_profile_save_dir}/encoder_layer_times_rank{torch.distributed.get_rank()}.csv"
                embedding_layer_times_path=f"{self.layer_profile_save_dir}/embedding_layer_times_rank{torch.distributed.get_rank()}.csv"
                
                with open(layer_times_path, 'a', newline='') as file:
                    writer = csv.DictWriter(file, fieldnames=layer_times.keys())
                    if not layer_header_written:
                        writer.writeheader()
                        header_written=True
                    writer.writerow(layer_times)
                    
                with open(embedding_layer_times_path, 'a', newline='') as file:
                    writer = csv.DictWriter(file, fieldnames=embedding_layer_times.keys())
                    if not emblayer_header_written:
                        writer.writeheader()
                        header_written=True
                    writer.writerow(embedding_layer_times)
        ...
        if self.pre_process:
            ...
            if self.layer_profile:
                e_emb.record()
                torch.cuda.synchronize()
                dec_embedding_layer_time = s_emb.elapsed_time(e_emb)
                embedding_layer_times["decoder_embedding"].append(dec_embedding_layer_time)
                if torch.distributed.get_rank() == 0:
                    print(f"Transformer Language Model dec_embedding_layer_time: {dec_embedding_layer_time}")                 
        else:
            decoder_input = None

        # Run decoder.
        decoder_output, *decoder_moe_losses, decoder_layer_times = self.decoder(
            decoder_input,
            dec_attn_mask,
        ...
        if torch.distributed.get_rank() == 0:
            print(f"Transformer Language Model decoder_layer_times: {decoder_layer_times}") 
        
        if self.layer_profile:
            if torch.distributed.get_rank() == 0:
                print(f"Transformer Language Model save en + de layer times") 
                
            layer_times = {"encoder_layer_times": encoder_layer_times, "decoder_layer_times": decoder_layer_times}
            
            import csv
            layer_header_written=False
            emblayer_header_written=False
            
            layer_times_path=f"{self.layer_profile_save_dir}/layer_times_rank{torch.distributed.get_rank()}.csv"
            print(f"[{torch.distributed.get_rank()}] layer_times_path: {layer_times_path}")
            
            embedding_layer_times_path=f"{self.layer_profile_save_dir}/embedding_layer_times_rank{torch.distributed.get_rank()}.csv"
            print(f"[{torch.distributed.get_rank()}] embedding_layer_times_path: {embedding_layer_times_path}")
