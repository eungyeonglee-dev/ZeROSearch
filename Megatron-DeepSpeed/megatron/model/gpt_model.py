...
class GPTModelPipe(PipelineModule,MegatronModule):
    ...
    def __init__(self,
        ...
        self.layer_profile_2 = args.layer_profile_2
        self.layer_profile_save_dir = args.layer_profile_save_dir
        
        if self.layer_profile_2:
        ...
        self.specs = []
        self.layer_times = {"layer_name":[], "layer_time": []}
        ...
        if self.layer_profile_2:
            ...
            if torch.distributed.get_rank() == 0:
                to_float16_layer_time = s.elapsed_time(e)
                self.layer_times["layer_name"].append("to_float16_layer")
                self.layer_times["layer_time"].append(to_float16_layer_time)
                print(f"to_float16_layer_time: {to_float16_layer_time}")
        ...
        if args.untie_embeddings_and_output_weights:
            ...
            if self.layer_profile_2:
                ...
                if torch.distributed.get_rank() == 0:
                    ...
                    self.layer_times["layer_name"].append("embedding_layer")
                    self.layer_times["layer_time"].append(embedding_layer_time)
                    print(f"embedding_layer_time: {embedding_layer_time}")
        ...
        for layer_idx in range(args.num_layers):
            ...
            if self.layer_profile_2:
                ...
                torch.cuda.synchronize()
                if torch.distributed.get_rank() == 0:
                    layer_time = s.elapsed_time(e)
                    self.layer_times["layer_name"].append("layer_time")
                    self.layer_times["layer_time"].append(layer_time)
                    print(f"[{layer_idx}] layer_time: {layer_time}")
        ...
        if self.layer_profile_2:
            ...
            torch.cuda.synchronize()
            if torch.distributed.get_rank() == 0:
                normlayer_time = s.elapsed_time(e)
                self.layer_times["layer_name"].append("layernorm")
                self.layer_times["layer_time"].append(normlayer_time)        
                print(f"layernorm time: {normlayer_time}")
                import csv
                layer_header_written=False
                
                layer_times_path=f"{self.layer_profile_save_dir}/layer_times.csv"
                
                
                with open(layer_times_path, 'a', newline='') as file:
                    writer = csv.DictWriter(file, fieldnames=self.layer_times.keys())
                    if not layer_header_written:
                        writer.writeheader()
                        header_written=True
                    writer.writerow(self.layer_times)
                print(f"save {layer_times_path} done")
