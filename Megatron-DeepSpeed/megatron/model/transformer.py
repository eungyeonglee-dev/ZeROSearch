        ...
        if self.layer_profile:
            ...
            if self.layer_profile_save:
                ...
                with open(csv_file, mode='a', newline='') as file:
                    ...
                    writer.writerow([self.layer_number, layer_time])
            else:
                if torch.distributed.get_rank() == 0:
                    print(f"[{self.layer_number}] layer_time: {layer_time}")
...
def _get_num_layers(args, model_type, is_decoder=False):
    ...
    if model_type == ModelType.retro_encoder:
        ...
        if is_encoder_and_decoder_model:
            ...
            if args.balance:
                balance=args.balance.split("-")
                print(f"[{torch.distributed.get_rank()}] {mpu.get_pipeline_model_parallel_rank()}")
                print(f"[{torch.distributed.get_rank()}] {balance}")
...
def get_num_experts_per_layer(num_experts: list, num_layers: int, expert_interval: int, offset: int = 0) -> list:
    ...
    for i in range(num_layers):
        ...
        # print(f"i: {i} layer_num: {layer_num} offset: {offset} expert_interval: {expert_interval}")
        # print(f"i:{i} num_layers:{num_layers} len(num_experts): {len(num_experts)} num_experts: {num_experts}")
        # print(f"(layer_num-1) // expert_interval: {(layer_num-1) // expert_interval}")
...
class ParallelTransformer(MegatronModule):
    ...
    def __init__(self, config,
        ...
        if self.num_layers == 0:
            ...
            # Build the layers
            n_e=1
            self.layers = [build_layer(i + 1 + offset,n_e=n_e) for i in range(self.num_layers)]
            '''
                ...
                if self.checkpoint_activations:
                    ...
                    for index in range(self.num_layers):
                        ...
                        if isinstance(hidden_states, tuple):
                            ...
                            # assert (len(hidden_states) == 3 or len(hidden_state) == 4)
        ...
        if self.layer_profile:
            ...
            if self.layer_profile_save:
                ...
                with open(csv_file, mode='a', newline='') as file:
                    ...
                    if not file_exists:
                        writer.writerow(['idx','layer_times'])
                    writer.writerow(["lmhead", lmhead_time])    
            else:
                print(f"[{torch.distributed.get_rank()}] LMHeadPipe_time: {lmhead_time}")
